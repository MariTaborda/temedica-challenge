...
async function main() {
    ...
    const mariaClaim1 = await prisma.claim.create({
        data: { patientId: maria.id, submissionDate: new Date("2023-01-10"), reimbursementDate: new Date("2023-01-25"), totalCost: 450 },
    });
    const mariaClaim2 = await prisma.claim.create({
        data: { patientId: maria.id, submissionDate: new Date("2023-03-15"), reimbursementDate: new Date("2023-03-30"), totalCost: 300 },
    });
    const alexClaim1 = await prisma.claim.create({
        data: { patientId: alex.id, submissionDate: new Date("2023-02-05"), reimbursementDate: new Date("2023-02-20"), totalCost: 500 },
    });
    const maxClaim1 = await prisma.claim.create({
        data: { patientId: max.id, submissionDate: new Date("2023-04-01"), reimbursementDate: new Date("2023-04-18"), totalCost: 200 },
    });
    const dianaClaim1 = await prisma.claim.create({
        data: { patientId: diana.id, submissionDate: new Date("2023-01-20"), reimbursementDate: new Date("2023-02-05"), totalCost: 720 },
    });
    const peterClaim1 = await prisma.claim.create({
        data: { patientId: peter.id, submissionDate: new Date("2023-03-10"), reimbursementDate: new Date("2023-03-25"), totalCost: 410 },
    });
    const elenaClaim1 = await prisma.claim.create({
        data: { patientId: elena.id, submissionDate: new Date("2023-04-05"), reimbursementDate: new Date("2023-04-20"), totalCost: 250 },
    });
